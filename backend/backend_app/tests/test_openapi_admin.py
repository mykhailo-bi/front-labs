...
class OpenApiAndAdminTests:
    ...
    def test_permission_classes_branches(self):
        ...
        assert not api_module.IsAuthenticated().has_permission(type("R", (), {"user": None})(), None)
        assert api_module.IsAuthenticated().has_permission(type("R", (), {"user": object()})(), None)
        assert api_module.IsAdmin().has_permission(type("R", (), {"user": type("U", (), {"is_admin": True})()})(), None)
